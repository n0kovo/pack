...
class StatsGen:
    ...
    def print_stats(self):
        ...
        for (length, count) in sorted(
            iter(self.stats_length.items()), key=operator.itemgetter(1), reverse=True
        ):
        ...
        for (char, count) in sorted(
            iter(self.stats_charactersets.items()),
        ...
        for (simplemask, count) in sorted(
            iter(self.stats_simplemasks.items()),
            key=operator.itemgetter(1),
            reverse=True,
        ):
            if self.hiderare and not count * 100 // self.filter_counter > 0:
        ...
        for (advancedmask, count) in sorted(
            iter(self.stats_advancedmasks.items()),
            key=operator.itemgetter(1),
            reverse=True,
        ):
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--output",
        dest="output_file",
        help="Save masks and stats to a file",
