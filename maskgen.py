...
class MaskGen:
    ...
    def loadmasks(self, filename):
        ...
        for (mask, occurrence) in mask_reader:
            ...
            if (
                ...
                and (
                    self.maxoccurrence is None or mask_occurrence <= self.maxoccurrence
                )
                and (
                    self.mincomplexity is None or mask_complexity <= self.mincomplexity
                )
                and (
                    self.maxcomplexity is None or mask_complexity <= self.maxcomplexity
                )
    ...
    def generate_masks(self, sorting_mode):
        ...
        for mask in sorted(
            self.masks.keys(), key=lambda m: self.masks[m][sorting_mode], reverse=True
        ):
...
if __name__ == "__main__":
    ...
    filters.add_option(
        ...
        type="int",
        metavar="3600",
        help="Minimum mask runtime (seconds)",
    )
    filters.add_option(
        "--maxtime",
        dest="maxtime",
        type="int",
        metavar="3600",
        help="Maximum mask runtime (seconds)",
    )
    filters.add_option(
        "--mincomplexity",
        dest="mincomplexity",
    ...
    sorting.add_option(
        ...
        action="store_true",
        dest="optindex",
        help="sort by mask optindex (default)",
        default=False,
    )
    sorting.add_option(
        "--occurrence",
    ...
    coverage.add_option(
        "--checkmasks",
        dest="checkmasks",
        help="check mask coverage",
        metavar="?u?l?l?l?l?l?d,?l?l?l?l?l?d?d",
    )
    coverage.add_option(
        "--checkmasksfile",
        dest="checkmasks_file",
        help="check mask coverage in a file",
        metavar="masks.hcmask",
    )
